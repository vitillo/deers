...
Linear, Embedding, RMSNorm, LayerNorm, ReLU, GELU, SiLU, SwiGLU, Dropout, Sequential, `Parameter` / `ParamStore` / `ParamBuilder`, `nn::functional` (dropout, causal_mask), CausalSelfAttention (MHA/GQA), MLP, GPT Block, GPT, KvCache, Qwen3Block, Qwen3 (incl. QK-Norm and RoPE scaling), MnistMLP.
...
cargo test --locked --test gqa --test kvcache --test qknorm --test silu_swiglu --test rope_scaling 2>&1 | tail -20
```
